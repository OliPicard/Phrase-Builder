...
namespace Phrase_Builder
{
    public partial class phraseBuilder : Form
    {
        ...
        {
            String[] NameListOne = new string[] { "Cosmic Panda", "Pizza Delivery Boy", "Boom Goes the Dinimatie","why is the rum gone?","Grumpy Cat","Serious Octopus","Alt+ F4","Tasty Pickle","NoNoNoNo Cat","Oh Long Johnson","Chocolate Rain"};
            Random pepperpig = new Random(); //create a new random.
            int length = NameListOne.Length; //get the length of the string
            int rand1 = (int)(pepperpig.Next(0,11)); //provide pepperpig(random) guidence.
